...
######################## Copy TT-Logix File Batch Start ########################
. $PARS_FILE_DIR/$1
...
if [ -z "$2" ]; then
	FILE_FILTER=MM15X4V1
fi

${JAVA_PATH}/java -Xms512M -Xmx1024M -jar ${JAVA_JAR_PATH}/${BATCH_JAR_FILE} ${BATCH_ID} ${INTERFACE_PATH} ${WORKING_PATH} ${BACKUP_PATH} ${FILE_FILTER} 2>&1 >> $BATCH_ERROR_LOG_DIR/$ERROR_FILE
ret=$?
