...
public class CsvReader {
    ...
    public static <T extends BaseCsvFileEntity> List<T> readFile(File file, Class<T> clsType,
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null ) {
                ...
                if (tempString.endsWith(StringConst.VERTICAL)) {
    ...
    private static boolean checkFileName(File file) {
        ...
        // check length
        if (fileName.length() != NumberConst.IntDef.INT_TWENTY_SEVEN) {
            logger.error(String.format("File name of file[%s] is incorrect. File name length is not 27.",
                new Object[] { file.getName() }));
            throw new BatchException();
        }

        // check file dates
        String fileDate = file.getName().substring(NumberConst.IntDef.INT_NINE, NumberConst.IntDef.INT_TWENTY_THREE);

        // format
        Date date = DateTimeUtil.parseDate(fileDate, DateTimeUtil.FORMAT_YYYYMMDD_HHMMSS);
        if (!StringUtil.isNumeric(fileDate) || date == null) {
            logger.error(String.format("File name of file[%s] is incorrect. Format of file create Date is incorrect.",
                new Object[] { file.getName() }));
            throw new BatchException();
        }
