 ...
 * @author ma_b
 */
package com.chinaplus.batch.common.util.interceptor;
...
public class ResourceBundleMessageSourceEx extends ResourceBundleMessageSource {
     ...
     */
    public void setMessageLocations(Resource[] messageLocations) throws IOException {
        ...
        if (this.messageLocations != null && this.messageLocations.length > 0) {
            for (Resource messageLocation : this.messageLocations) {
                ...
                String fileName = "";
                try {
                    fileName = messageLocation.getURL().getPath();
                } catch (IOException e) {
                ...
                if (!baseNames.contains(baseName)) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("Parsed message resource file: {}" + baseName, baseName);
                    }
                }
            }
        }
    ...
    private static String getBasenameFromLocation(String location) {
        ...
        if (!StringUtil.isEmpty(location)) {
            ...
            if (location.indexOf(basePath) >= 0) {
                int idx = location.indexOf(basePath);
                int offSet = idx + basePath.length();
                baseName = location.substring(offSet);
            }
            if (baseName.indexOf(".jar") >= 0) {
                int offSet = baseName.indexOf("/");
                if (offSet > 0) {
                    baseName = baseName.substring(offSet + IntDef.INT_ONE);
                }
            }
