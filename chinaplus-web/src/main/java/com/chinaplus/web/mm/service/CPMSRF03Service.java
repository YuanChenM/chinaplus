 ...
 */
@Service
public class CPMSRF03Service extends BaseService {
    ...
    /**
     * get CPMSRF03Entity List
     * 
     * @param impOfficeId impOfficeId
     * @param shipRouteCode shipRouteCode
     * @param effFromEtd effFromEtd
     * @param effToEtd effToEtd
     * @return CPMSRF03EntityList CPMSRF03EntityList
     */
    public List<CPMSRF03Entity> getCPMSRF03EntityList(String impOfficeId, String shipRouteCode, Date effFromEtd,
        Date effToEtd) {
        ...
        List<CPMSRF03Entity> CPMSRF03EntityList = baseMapper.select(this.getSqlId("getCPMSRF03EntityList"), param);

        return CPMSRF03EntityList;
    }
            ...
            if (data != null) {
                ...
                for (int j = 0; j < rowItemCount; j++) {
                    ...
                    if (templateCells != null && j < templateCellCount && null != templateCells[j]) {
                        ...
                        if (j == IntDef.INT_SEVEN) {
                            int number = rowNum + IntDef.INT_ONE;
                            String formulaQ = new StringBuilder().append("IF(").append("G").append(number)
                                .append("=\"\", \"\",").append("TEXT(WEEKDAY(").append("G").append(number)
                                .append("),\"(ddd)\"))").toString();
                            cell.setCellFormula(formulaQ);
                        } else if (j == IntDef.INT_TWELVE) {
                            String formulaQ = "IF(L6=\"\",\"\",IF(MOD(L6+N6,7)=0,7,MOD(L6+N6,7)))";
                            int number = rowNum + IntDef.INT_ONE;
                            formulaQ = formulaQ.replaceAll("L6", "L" + number);
                            formulaQ = formulaQ.replaceAll("N6", "N" + number);
                            cell.setCellFormula(formulaQ);
