...
/**
 * Download Revised Shipping Status (Doc3) Controller.
 */
@SuppressWarnings({ "deprecation", "unchecked", "unused" })
@Controller
public class CPSSMF03Controller extends BaseFileController {

    /** The begin of download constant define */
    private static final String DOWNLOAD_ZIP_FILE_NAME = "Shipping Status Doc3_{0}.zip";
    private static final String ZIP_CONTENT_TYPE = "application/x-zip-compressed;charset=UTF-8";
    private static final String DOWNLOAD_EXCEL_FILE_NAME = "Shipping Status Doc3_{0}.xlsx";
    private static final String DOWNLOAD_EXCEL_SHEET_NAME = "Shipping Status Doc3";
     ...
     */
    @RequestMapping(value = "/sa/CPSSMF03/downloadcheck")
    @ResponseBody
    public BaseResult<String> downloadCheck(@RequestBody PageParam param, HttpServletRequest request,
        ...
        if (validResult) {
            ...
            List<TntSsPlan> planList = this.cpssmf03Service.searchSsPlanList(param);
            ...
            // search base info
            List<CPSSMF01Entity> planInfoList = this.cpssmf03Service.searchShippingPlanInfo(param, orderList,
     ...
     */
    @RequestMapping(value = "/sa/CPSSMF03/download")
     ...
     */
    public void outputMainProcess(BaseParam param, Workbook wbTemplate, SXSSFWorkbook wbOutput, CPSSMF03Service service) {
        if (null == cpssmf03Service) {
            cpssmf03Service = service;
        }
        Date officeTime = DateTimeUtil.parseDate(DateTimeUtil.formatDate(null,
            cpssmf03Service.getDBDateTime(param.getOfficeTimezone())));
    ...
    private void doOutput(BaseParam param, Date officeTime, Workbook wbTemplate, SXSSFWorkbook wbOutput) {
        ...
        if (null != planList && !planList.isEmpty()) {
            orderList = this.cpssmf03Service.searchOrderList(param);
            orderListByPlan = this.cpssmf03Service.searchOrderListByPlan(param);
        }
        ...
        // multi ipo and customer order number
        boolean multiIpo = false;
        ...
        if (null != planInfoList && !planInfoList.isEmpty()) {
            invoiceInfoList = this.cpssmf03Service.searchInvoiceInfo(param, planInfoList);
        }
        ...
        Map<String, Integer> diffPlanMap = this.cpssmf03Service.sortPlanAndActualInfo(planInfoList, invoiceInfoList,
            planAndActualList, nirdList, officeTime);
        ...
        int columnNum = this.outputPlanAndActualInfo(param, planAndActualList, new Object[] { uomMap, rowNumMap,
            ipoRowNumMap, summaryQtyMap, diffPlanMap, samePoRowNumMap }, wbTemplate, sheet, maxRowNum, multiIpo);
        if (!multiIpo) {
    ...
    private void outputFooter(BaseParam param, Workbook wbTemplate, Sheet sheet, int maxRowNum) {
        ...
        String downloadTime = DateTimeUtil.formatDate(dateTimeFormat,
            this.cpssmf03Service.getDBDateTime(param.getOfficeTimezone()), lang);

        param.setFilter("officeId", param.getCurrentOfficeId());
        Map<Integer, String> lastSyncTimeMap = this.cpssmf03Service.searchDataSyncTime(param, dateTimeFormat, lang);
    ...
    private void outputNirdColumn(BaseParam param, List<CPSSMF01ColEntity> nirdList, Object[] maps,
        Workbook wbTemplate, Sheet sheet, int[] position) {
        ...
        for (int i = 0; i < nirdList.size(); i++) {
            ...
            if (i == nirdList.size() - IntDef.INT_ONE) {
                mostRight = true;
            }
    ...
    private int outputPlanAndActualInfo(BaseParam param, List<CPSSMF01ColEntity> planAndActualList, Object[] maps,
        Workbook wbTemplate, Sheet sheet, int maxRowNum, boolean multiIpo) {
    ...
    private void outputPlanAndActualData(BaseParam param, CPSSMF01ColEntity item, Object[] maps, Workbook wbTemplate,
        ...
        this.outputPlanOrActualQtyInfo(item, maps, wbTemplate, sheet, new int[] { columnNum, maxRowNum },
            new boolean[] { isPlan, false, multiIpo });
        columnNum++;
    ...
    private void outputPlanOrActualQtyInfo(CPSSMF01ColEntity item, Object[] maps, Workbook wbTemplate, Sheet sheet,
        ...
        Map<String, Integer> diffPlanMap = (Map<String, Integer>) maps[IntDef.INT_FOUR];
        Map<String, List<Integer>> samePoRowNumMap = (Map<String, List<Integer>>) maps[IntDef.INT_FIVE];
        ...
        boolean multiIpo = flag[IntDef.INT_TWO];
        ...
        if (null != partsList && !partsList.isEmpty()) {
            for (CPSSMF01PartEntity entity : partsList) {
                ...
                if (modColumnFlag) {
                    ...
                    if (isPlan) {
                        if (null != diffPlanMap.get(bfDiffPlanKey.toString())
                                && item.getRevisionVersion().intValue() == diffPlanMap.get(bfDiffPlanKey.toString())
                                    .intValue()) {
